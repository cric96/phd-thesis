...
Despite the diverse hardware configurations, nodes within a CPSW (Cyber-Physical Software) display consistent behaviour at the local level.
 They run the same local algorithms, thereby achieving the system's collective objectives in a standardized way. 
 However, it is important to clarify that ``homogeneous behaviour'' does not imply synchronized actions across all nodes. 
 Given the system's inherently distributed and potentially expansive nature, 
...
While these optimization algorithms offer valuable methodologies and have broad applicability, they are not the central focus of this thesis. 
 ...
 By doing so, the thesis contributes to the development of more robust, resilient, and adaptive collective systems.
...
In summary, 
 ...
 However, the unique challenges and complexities of \acp{cpsw} require further innovation and adaptation, particularly in the areas of automatic design and optimization. 
 By integrating these advanced techniques, we can create more robust, adaptable, and intelligent \acp{cpsw} that can operate effectively in a wide range of applications and environments.
...
\ac{cpsw} can be viewed as a subset of CAS, 
 but with two distinctions: 
  i) the entities are \emph{cyber-physical}, and 
  ii) even though the devices are heterogeneous, the behaviour is homogeneous. 
These two components significantly influence the design process, 
which is why this thesis have identified this subclass of systems.
%
%\printbibliography

\section{Wrap up}
In conclusion, \acf{cpsw} represent a specialized subset of Collective Adaptive Systems (CAS), distinguished by their cyber-physical nature and homogeneous behaviour despite device heterogeneity. 
...
These challenges necessitate innovative approaches in automatic design, optimization, and real-time adaptability.
