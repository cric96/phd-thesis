...
\chapter{Dynamic Decentralization Domains} %for the Internet of Things}
Edge computing
...
In this paper, we address the problem of capturing the right abstractions for modeling \acp{dcp},
...
environmental (landslides, floodings, fires) monitoring and response,
...
Every device %, at some time, 
 may interact with a limited subset of other devices (its \emph{neighbors}).
%
...
% In particular,
We target the idea of programming the overall behavior
of such systems 
...
\subsection{Decentralized situation recognition and action: a case study}
...
\paragraph{\reqlabel{req-concurrency}{R1} Concurrent collective task execution.} 
%
In \casename{},
...
\paragraph{\reqlabel{req-flexible-dec}{R2} Flexible and adaptive decentralization.} 
%
\casename{}
is centered on organizing distributed sensing and actuation
...
\paragraph{\reqlabel{req-feedback}{R3} Feedback-regulated activity within decentralized domains.}
%
...
\begin{figure*}
  ...
  \caption{
    ...
    with colors denoting environmental phenomena. % in terms e.g. of measured data or computed data associated to spatial locations.
...
stance
 by which the behavior of a whole system
...
So, we aim at a \emph{macro}-programming model~\cite{DBLP:journals/corr/abs-2201-03473},
 ...
 defines the behavior of the whole system
...
\emph{sensing, processing, and neighbor-based communication},
...
We also want the \ac{api} to be \emph{declarative},
 ...
 promoting the behavior of the abstractions
...
\item concerning \acp{cct} (cf. \ref{req-concurrency})
    ...
    where each sensing domain has a \emph{center} and an \emph{extension} in space;
%    \meta{a CC model a DD? should improve on the relationship between the two}
    \item both the extension in space and the center can change dynamically to improve the way the underlying phenomenon is being tracked,
...
%    \meta{a small comment on why?}
    ...
    \item the decentralized sensing will output the collectively-sensed result and
      the identifier of the device closer to the area center;
...
progressively building the intended global behavior.
...
\begin{figure}[t]
  ...
  \caption{
    ...
    their colors map the \acp{dd} they are subject to when measuring rainfall intensity.
...
\begin{figure*}[t]
  ...
  \caption{
    \revise{
      ...
      Charts show risk (the darker, the higher) as estimated in realtime by an oracle using altitude and rainfall intensity.
      Stations are depicted with a red circle, and those alerted are filled in white.
      Solid black areas are non-land.
      Alerted stations are indeed those closest to the zones of highest risk.
  }}
...
%
Yet, contributions on patterns and programming abstractions
