...
\chapter[Language: Reactive-based collective computations]{
  Language: SReactive-based collective computations
}\label{chap:eng:frp}%\mtcaddchapter %% Todo improve
...
As our language-based approach, in this chapter, we focus on an approach to self-organization,
...
and whereby each sub-expression is amenable to independent scheduling:
 ...
 flexibility in the sensing/actuation model, and execution efficiency. %sec:contrib
 
\section{Motivation}\label{acsos2023-frp:sec:background}
In this section,
...
\section{FRASP Programming Model}
...
In this thesis, we adopt the general system model devised in \Cref{part:background},
...
%
Communication with neighbours is handled \emph{in both directions at once} through a single construct, \lstinline|nbr(f)|, which takes a flow \texttt{f} as a parameter.
%
...
Notice that, upon a change in the value sensed by \texttt{colour},
 a device may dynamically switch branches and hence sub-computation domain and ``aligned'' neighbour set.
...
Multiple gradient computation algorithms exist~\cite{audrito2017ULT} as previously outlined. 
...
\begin{figure}
  ...
  \caption[The reactive dataflow graph corresponding to the channel example.]{
      The reactive dataflow graph corresponds to \Cref{acsos2023-frp:ex:channel}.
      %
      ...
      a red block depends on changes corresponding to neighbours' red blocks and is communicated via message passing.
  }
...
\begin{itemize}
    ...
    \item \emph{Interact only upon relevant changes}: each device should avoid broadcasting an export that has not changed since the last one, with the direct consequence that no further message exchange should be required if a computation reaches a stable configuration.
\end{itemize}
...
The design is organised into three packages:
    ...
    and \texttt{simulation},
        ...
        (for more advanced support, we also integrated \ac{langname} into Alchemist---see \Cref{acsos2023-frp:sec:eval}).
...
%
The former represents a minimally complex self-organising behaviour, enabling the evaluation of basic dynamics, 
...
For \emph{scenario G},
 a group of nodes (i.e., a $20m \times 20m$ area at the bottom left) is marked as a gradient source.
%
...
This result is expected:
 ...
 each message different from the previous one
...
sending approximately 40\% fewer messages.
%
...
\begin{figure}
  ...
  \caption[Simulation results of \ac{langname} simulations]{
    Simulation results of \ac{langname} simulations.
    %
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption[behaviour of the channel in response to changes in a destination]{behaviour of the channel in response to changes in a destination}
