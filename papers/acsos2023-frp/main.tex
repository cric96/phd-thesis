...
``guided self-organisation''~\cite{prokopenko2009guided-selforg},
``controlled self-organisation''~\cite{DBLP:journals/taas/SchmeckMCMR10}, and
...
In this work, we focus on the \emph{programming language-based} approach to self-organisation,
...
Motivated by this, and inspired by the functional reactive paradigm,
 in this work 
 \emph{we propose a reactive self-organisation programming language
...
\item provide an open-source implementation as a Scala \ac{dsl}\footnote{\label{footnote:dsl}\url{https://github.com/cric96/distributed-frp}},
...
through an open-source, permanently available artefact with reproducible simulations\footnote{\label{footnote:eval}\url{https://github.com/AggregateComputing/experiment-2023-acsos-distributed-frp}}.
\end{itemize}
%
The result is a functional reactive self-organisation programming model, 
...
%
\Cref{sec:background} covers background on self-organisation programming and the functional reactive paradigm.
%
\Cref{sec:contrib} presents the \ac{langname} programming model.
%
\Cref{sec:impl} details the implementation.
%
\Cref{sec:eval} describes the experimental setup and results.
%
\Cref{sec:conc} draws conclusions and future research work.
...
In this section,
 we review approaches for self-organisation engineering (\Cref{sec:background:selforg}),
...
%
Then, we provide background on \ac{frp} (\Cref{sec:background:frp}),
...
\subsection{Self-organisation Engineering Approaches}
\label{sec:background:selforg}
...
The first distinction in approaches for self-organisation engineering
 is between automatic 
 and manual approaches.
%
In the former category, 
 the control logic of the agents
 is learned, synthesised, or evolved automatically---as e.g., in multi-agent reinforcement learning~\cite{DBLP:journals/corr/abs-1911-10635},
...
Among reactive approaches is \emph{Tuples On The Air (TOTA)}~\cite{tota},
 a programming model for decentralised peer-to-peer networks of mobile nodes or agents.
%
...
Indeed, it emerged as 
 ...
 self-organisation~\cite{vbdacp:ac:survey:jlamp},
...
Instead, more compositional languages like, e.g., aggregate computing languages~\cite{vbdacp:ac:survey:jlamp,DBLP:journals/nca/BachrachBM10,DBLP:conf/ecoop/AudritoCDSV22} and Buzz~\cite{DBLP:conf/iros/PinciroliB16},
 ...
 by composing \emph{blocks} of simpler self-organising behaviours---see \Cref{paradigmatic-examples} for examples.
...
An example of such a library %  language 
 ...
 which we leveraged to implement \ac{langname} as described in \Cref{sec:impl}.
...
% which we leveraged for the implementation of \ac{langname} as described in \Cref{sec:impl}.
...
%
First, we explain the system and execution model (\Cref{sec:sys-model});
 then, we present the language abstractions and primitives (\Cref{programming-constructs});
 finally, we show how paradigmatic examples of self-organisation can be expressed in \ac{langname} (\Cref{paradigmatic-examples}).
...
%
This idea will be shown in \Cref{ex:channel} and \Cref{fig:channel:graph}.

Notice that the model is logical 
 and may be implemented using different approaches and optimisations---e.g., a device may send a heartbeat to notify that it is still a neighbour and its data has not changed,
...
a \lstinline|Context| and returns a \emph{cell} of \lstinline|Export|s, possibly depending on the exports of other \lstinline|Flow|s, recursively---see \Cref{sec:impl:impl} for details.
%
...
%(\Cref{fig:semantics-mux}) is an alternative to \texttt{branch} that introduces conditional behaviour without resulting in partitions in the device network.
%
...
%
%Neighbouring constructs, i.e., \texttt{nbr} and \texttt{nbrSensor}, require knowing which neighbours are aligned when they get evaluated. %, as shown in \Cref{fig:semantics-neighboring}.
%
...
(how such a sensor works is an implementation detail---e.g., it may use GPS traces or WiFi signal strength).
%
%\begin{figure}
%    \centering
%    \begin{subfigure}[b]{0.47\textwidth}
...
% 
Notice that, upon a change in the value sensed by \texttt{color},
...
%%
%The specification states that the \quotes{off-by-one} \texttt{Flow} should be constructed by aligning with the previous state of the same device, which can be read from the context leveraging the fact that every device is a neighbor of itself. % (\Cref{fig:semantics-loop-next}).
%%
...
\label{paradigmatic-examples}

In this section, we cover two fundamental self-organising behaviours
that will be exercised in the evaluation in \Cref{sec:eval}.
%
...
\begin{example}[Self-Healing Gradient]\label{ex:g}
%
...
%
\Cref{fig:scenario-g} provides a representation of a gradient.
...
\begin{example}[Self-Healing Channel]\label{ex:channel}
%
...
def channel(
  ...
  gradient(source), %\label{chan:g:src}%
  gradient(destination), %\label{chan:g:dest}%
  distanceBetween(source, destination) %\label{chan:distbetw}%
){
  (distSource, distDest, distBetween) =>
    distSource + distDest <= distBetween + width%\label{src:chan:expr}%
}
\end{lstlisting}
% NB: it should actually be liftTwice(distances, nbrRange(), values)(_ + _)
\end{minipage}
%
This self-organising data structure can be implemented by leveraging two \lstinline|gradient|s (one from the source and one from the destination---cf. \Cref{chan:g:src,chan:g:dest})
...
a structure similar to the \lstinline|gradient| implementation in \Cref{ex:g}) that supports the computation of \lstinline|distanceBetween|.
%
The \lstinline|channel| depends on these three flows: i.e.,
the expression at \Cref{src:chan:expr}
will be re-evaluated only upon a change of the output of (one of) these three sub-computations.
%
For a graphical view of the local and distributed dependency graph, see \Cref{fig:channel:graph}: the key idea is that, e.g., a local change in sensor \lstinline|source| will not cause a re-computation of \lstinline|gradient(destination)|.
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption{
      The reactive dataflow graph corresponding to \Cref{ex:channel}.
      %
      \Cref{fig:channel-graphical} provides the local view of the computation for a single node
      (where the layers denote different semantic kinds of dependencies),
      whereas \Cref{fig:interactions} shows the \emph{distributed} dependency graph.
      %
...
This section briefly provides the implementation goals (\Cref{sec:impl:goals}), architectural design (\Cref{sec:impl:arch}), and implementation details (\Cref{sec:impl:impl}) of \ac{langname}.
%
Even though a complete description of the implementation is beyond the aims of this paper,
 ...
 and to provide general guidance for understanding the code organisation of the provided artefact (see \Cref{footnote:dsl}).
...
%
This vision can be summarised with the term \textit{functional reactive self-organisation}.
%
Considering the system model introduced in \Cref{sec:sys-model},
...
The architecture of the prototype is shown in \Cref{fig:arch}.
%
The design is organised into three packages:
    ...
    and \texttt{simulation},
        ...
        (for more advanced support, we also integrated \ac{langname} into Alchemist~\cite{PianiniJOS2013}---see \Cref{sec:eval}).
...
The design of the \ac{langname} \ac{dsl}
 is detailed in \Cref{fig:dsl-design}.

Following the system/execution model described in \Cref{sec:sys-model},
...
\texttt{Flow} is the type 
 ...
 and returns \texttt{Cell} (i.e. a time-varying value---cf. \Cref{sec:background:frp}) of \texttt{Export}.
%
Each \texttt{Language} construct
 ...
 whose nodes will execute as a response to change (cf. \Cref{fig:channel:graph}).
%
...
(see \Cref{footnote:eval}) using the \emph{Alchemist} simulator for large-scale pervasive systems\footnote{\url{https://alchemistsimulator.github.io/}}~\cite{PianiniJOS2013}.
%
In particular, 
...
as a reference for round-based computations (cf. \Cref{sec:background:selforg}).

\subsection{Goals}\label{eval:goals}
%
...
\item[G.3)] \emph{Reactivity}: the programs should react to various sources of change
 (e.g., network topology, sensor data, dependent computations---cf. \Cref{sec:sys-model}),
...
%      The outer circle represents sensor information, with yellow nodes indicating the presence of a source and green nodes indicating the presence of a destination. 
    ...
    \label{fig:scenario-screenshots}
\end{figure}
...
%
Each simulation is characterised by the mode of execution of the aggregate program,
 which can be 
...
The ``reactive with throttling'' policy
 ...
 parametrised on the throttling frequency $\gamma$
...
In the above setup, the two programs introduced in \Cref{sec:contrib},
 i.e., the self-healing gradient (\emph{scenario G}---cf. \Cref{fig:scenario-g}) and channel (\emph{scenario C}---cf.~\Cref{fig:scenario-c}),
...
\subsection{Results and Discussion}\label{eval:results}
%
In this section, we will present the results obtained from the simulations, 
 highlighting how the proposed model satisfies the goals elicited in \Cref{eval:goals}.
%
We run a total of 768 simulations
by running 64 randomised instances
...
%
Our results are presented in \Cref{fig:rel-values} and \Cref{fig:subprogram-eval}.
%\Cref{fig:channel-messages-rel} and \Cref{fig:gradient-messages-rel} for the gradient and the channel, respectively, averaging the data extracted.
%%% GOAL 1 

\subsubsection{Correctness (goal G.1)} Consider \Cref{fig:channel-output-rel} and \Cref{fig:gradient-output-rel}:
...
(\Cref{fig:channel-messages-rel,fig:gradient-messages-rel})
and time efficiency (\Cref{fig:gradient-output-rel,fig:gradient-output-rel}).
%
...
This can be observed from 
  \Cref{fig:channel-messages-rel} and \Cref{fig:gradient-messages-rel}:
...
%
\Cref{fig:subprogram-eval} shows how
...
(cf.\ the larger red circles in \Cref{fig:subprogram-eval}).
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{
  The behaviour of the channel in response to changes in a destination is shown through a sequence of snapshots in the first line (using the same graphical notation as \Cref{fig:scenario-screenshots} where, additionally, the grey shadows denote nodes where computation is taking place). 
...
The language is designed by taking inspiration 
 ...
 and reactive approaches to self-organisation like TOTA,
...
In future work,
 we would like to formalise a core calculus capturing behaviour and properties of \ac{langname} (similarly to field calculi and related formal languages \cite{vbdacp:ac:survey:jlamp,DBLP:conf/ecoop/AudritoCDSV22,DBLP:journals/lmcs/AudritoCDV23}),
 and explore how to support flexibility in actuation models (cf. swarm robotics).
Also, it would be interesting 
  to study the deployment of \ac{langname} applications, e.g.\ by considering how reactive dynamics and consumption profiles may interact with application partitioning strategies like the pulverisation model~\cite{CPPVW-FI2020} and deployment options.
