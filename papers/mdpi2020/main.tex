...
\subcaption{\label{fig:crowd-snapshot}A simulation snapshot of the crowd detection and steering example. Red nodes are devices in an overcrowded area;
...
\caption{\label{fig:eval:diagram}\revision{Diagram corresponding to the code below. This figure uses the same notation of \Cref{fig:crowd-diagram}. In addition, we use a same colour to denote multiple references to the same functional block.}}
\end{subfigure}
%%%%%
...
\caption{\revision{Average error (RMSE) for each combination of \emph{p} and \emph{healer count}.
