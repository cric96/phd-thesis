...
One is the field of \emph{autonomic computing}~\cite{DBLP:journals/computer/KephartC03} and \emph{self-adaptive systems}~\cite{lemos2010sweng-selfadaptive-2ndresearchroadmap},
 which promote the development of ICT \revision{(Information and Communications Technology)} systems
...
\item we analyse the aggregate computing framework by the perspective of autonomy, by covering its positioning with respect to individual and collective autonomy, and showing how it can supports adjustable autonomy \revision{(\Cref{s:autonomy-ac})};
...
%%
\revision{Finally, we conclude with a wrap-up in \Cref{s:conc}.}
%
...
%
At a second level, concrete and (semi-)formal notions of autonomy are developed to support engineering tasks under specific viewpoints\revision{---see \Cref{table:autonomy-notions} for a summary}.
%
...
\begin{tabularx}{\textwidth}{|p{0.355\textwidth}|X|X|X|X|}
\hline
...
%\\\hline
%& \multicolumn{2}{|c|}{\emph{weak agency}} & \multicolumn{2}{c|}{\emph{strong agency}}
%\\\hline
\end{tabularx}
\caption{\revision{A summary of common notions of autonomy.}}
...
%
Additionally, ScaFi also represents an agile framework for testing experimental language features (cf. \emph{aggregate processes}~\cite{DBLP:journals/eaai/CasadeiVAPD21}---referenced in \Cref{gaps:expressiveness}).
%
...
Such an abstract aggregate execution protocol %(introduced in \Cref{background-ac}---comprising context evaluation, aggregate program evaluation, context action) 
 can effectively be used to define an \emph{agent control architecture} (\Cref{fig:aggregate-control-arch}).
%
...
As a running example, consider a crowd detection and steering application~\cite{DBLP:journals/computer/BealPV15} (cf. \Cref{fig:crowd-snapshot}).
%
...
In ScaFi, such a program may be implemented, reusing a library of general-purpose aggregate components~\cite{DBLP:conf/saso/BealV14a}, %\cite{DBLP:conf/saso/FranciaPBV17,DBLP:journals/tomacs/ViroliABDP18}, 
 as per \Cref{fig:scafi-crowd}. 
%
Function \lstinline|crowdTracking| (\Cref{line:crowdtrack})
 runs
 a collective crowding risk estimation process,
 by calling \lstinline|collectiveDensityEstimation| (\Cref{line:colldens}),
...
%
Then, function \lstinline|crowdDispersal| (\Cref{line:dispersal})
...
\subcaption[A simulation snapshot of the crowd detection and steering example.]{\label{fig:crowd-snapshot}A simulation snapshot of the crowd detection and steering example. Red nodes are devices in an overcrowded area;
...
\subcaption{
\label{fig:crowd-diagram}\revision{Diagram corresponding to the ScaFi application code below. Boxes denote computational field expressions; solid arrows denote input/output relationships;
...
\end{lstlisting}
\subcaption{\label{fig:scafi-crowd}Aggregate program as implemented in ScaFi.}
%%
%%def localDensityEstimation(p: Double, r: Double, w: Double): Double = {
%%  val nbrsWithinRange = sumHood(mux(nbrRange() < r) { 1 } { 0 })
%%  nbrsWithinRange / (p * Math.PI * r * r * w)
%%}
%%
%%def recentlyTrue(event: Boolean, timeout: Double): Boolean = 
%%  branch ( event ) { true } { timerLocalTime(timeout) > 0 }
%
%%def collectiveDensityEstimation(p: Double, r: Double, thDanger: Double) = {
%%  val mr = managementRegions(r*2, nbrRange)
%%  val danger = average(mr, densityEst(p, r)) > thDanger &&
%%    summarize(mr, (_:Double)+(_:Double), 1 / p, 0) > 300
%%  if(danger) { high } else { low }
%%}
%%def collectiveDensityEstimation(p: Double, r: Double, w: Double, t: Double, 
%%                                thCrowd: Double, thDanger: Double): Boolean = {
%%    val partition = S(range, nbrRange)
%%    val localDensity = densityEstimation(p, range, w)
%%    val avg = summarize(partition, _+_, localDensity, 0.0) /
%%      summarize(partition, _+_, 1.0, 0.0)
%%    val count = summarize(partition, _+_, 1.0 / p, 0.0)
%%    avg > dangerousDensity && count > groupSize
%%%%}
%%def collectiveDensityEstimation(p: Double, r: Double, w: Double, t: Double, 
%%                                thCrowd: Double, thDanger: Double): Boolean = {
%%    val partition = S(r * 10) // split
%%    val localDensity = densityEstimation(p, r, w)
%%    val avg = summarize(partition, _+_, localDensity, 0.0) /
%%      summarize(partition, _+_, 1.0, 0.0)
%%    val count = summarize(partition, _+_, 1.0 / p, 0.0)
...
%
\revision{\Cref{table:summary-ac-individual-autonomy} synthesises the various sources and forms of individual autonomy in aggregate computing.}
...
In aggregate computing, the shared goal 
 ...
 are reified into an aggregate program (cf. \Cref{fig:scafi-crowd}),
...
For instance, in the crowd example,
 ...
 (cf. \Cref{line:on-collective-intention} in \Cref{fig:scafi-crowd}).
...
%
Consider the \emph{Self-organising Coordination Regions (SCR)} pattern~\cite{casadei2019scr}, which is also exploited in \Cref{s:eval}; a general encoding in ScaFi is as follows:
%
\begin{lstlisting}
...
\item \emph{adjustable autonomy}: by using structures or branching mechanisms to regulate the relationship between individual and collective autonomy, or by controlling the amount of endogenous vs. exogenous autonomy \revision{(cf. \Cref{table:summary-ac-individual-autonomy})}.
\end{itemize}
%
...
MOISE is a very articulated approach 
 for describing agent \emph{organisations}
...
Our basic requirements for a simulator
 ...
 the aggregate execution protocol discussed in \Cref{contrib-ac-control-arch}, which consists of asynchronous rounds of execution with neighbourhood-based communication.
%
...
\revision{In this experiment, \emph{Mobile nodes} can perform \emph{tasks}, namely problems to be solved.
 Our characterisation of this concept is a lightweight version of the definition in~\cite{DBLP:conf/agents/FatimaW01}: 
...
%
With ScaFi, this behaviour can be synthesised as described in \Cref{fig:wildlife-code}
%
\begin{figure}[t!]
\centering
...
\caption[High-level diagram expressing the implemented scenario]{\label{fig:eval:diagram}\revision{Diagram corresponding to the code below. This figure uses the same notation of \Cref{fig:crowd-diagram}. In addition, we use a same colour to denote multiple references to the same functional block.}}
\end{subfigure}
%%%%%
...
We also introduce a metric that represents the general performance of a simulation run.
 ...
 Given a sample t, events measure how many animal turn on danger during the simulation until the t. 
...
%
 ...
 The data reported in \Cref{fig:wildlife-simulation} contains the average behaviour of those simulations. 
% 
\revision{In \Cref{fig:stack-plot-error}, the average error is reported for each combination of \emph{healer count} and \emph{p}.}
%
...
%
\revision{We see the benefits of collective choice with higher healer count values (\Cref{fig:stack-plot-error}).
...
In the following,
 ...
 comprise
 expressing collective autonomous behaviour (\Cref{gaps:expressiveness})
 and
...
The case study of \Cref{s:eval} shows how the discussed framework
 ...
 as well as parameterised behaviours
