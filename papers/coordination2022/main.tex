...
%\meta{
%  \begin{itemize}
%    \item discussion on CAS engineering
%    \item discussion on top-down-global-to-local approaches
%    \item discussion on learning applied to CAS
%    \item few sentences that motivate RL + AC
%  \end{itemize}
%}
...
\acp{cas} are characterised 
 ...
 via local decision-making and decentralised interaction.
%
The \emph{engineering of \acp{cas}} is an open research problem~\cite{DBLP:journals/sttt/NicolaJW20,DBLP:journals/corr/abs-1108-5643} of significance, tightly linked with the problems of ``steering'' self-organisation and ``controlling'' emergence to promote desired while avoiding undesired emergents~\cite{DBLP:books/sp/08/Muller-SchloerS08}.
%
...
Besides the programming model and its implications,
 ...
 %for 
 distributed summarisation~\cite{DBLP:journals/cee/AudritoCDPV21}.
%
...
Specifically, we adopt a \emph{\ac{rl}-based approach}---where an agent learns from experience
 how to behave in order to maximise delayed cumulative rewards~\cite{sutton2018reinforcement-learning}.
%
...
\Cref{s:background} offers background on \ac{ac} and \ac{rl};
%
\Cref{s:contrib} discusses the integration of \ac{rl} and \ac{ac}, and the use of the approach to improve the basic gradient algorithm;
%
\Cref{s:eval} provides an experimental evaluation of the proposed approach;
%
\Cref{s:conc} summarises results and future research.
...
%%%
%%However, as will be discussed in \Cref{s:contrib},
%% learning could play a role to improve ...

We recap \acl{ac} in \Cref{s:background:ac}.
Then, we review \ac{rl} in \Cref{s:background:rl},
 to prepare the ground for our contribution, \emph{Reinforcement Learning-based Aggregate Computing} (\Cref{s:contrib}).
...
In the following, we summarise the computation model and the \ac{fc}/\scafi{} language, which are essential to understand the contribution and case study.
...
%
As a key example, we introduce the gradient algorithm, which will be considered \Cref{s:contrib,s:eval}.
...
%
The goal of \ac{rl} is to \emph{learn} a policy \emph{$\pi^*$} that maximises the long-term return $G$ (i.e. the cumulative reward) through a \emph{trial-and-error} process. 
%
...
%%
Finally, the \ac{rl} goal can be expressed as the maximisation of the \emph{expected} long-term return following a policy $\pi$:
%%
\begin{equation}
...
\end{equation}
%%
$Q^\pi$ is the corresponding value fuction that evaluates \emph{state-action} pairs:
%%
\begin{equation}
...
%  \end{cases}
%\end{align}
In this study (cf. \Cref{s:eval}), we use \ac{hql} as reference \ac{rl} algorithm since it has a strong empirical track for cooperative multi-agent systems~\cite{DBLP:journals/ker/MatignonLF12,DBLP:journals/tsmc/XuZLF12,DBLP:journals/asc/BarbaliosT14}. 
%
...
%
Novel approaches are structured in between independent learners and centralised learners techniques---the so-called \ac{ctde}~\cite{DBLP:phd/ethos/Foerster18}.
...
\section{Reinforcement Learning-based Aggregate Computing}\label{s:contrib}
%\meta{
%  \begin{itemize}
...
As anticipated in \Cref{s:background:ac}, the behaviour of an aggregate system depends on the interplay of three main ingredients:
...
So, our long-term goal is to integrate \ac{rl} into the \ac{ac} ``stack'' (i.e., across the platform, language, and library levels) to improve the collective behaviour defined by \scafi{} aggregate programs in terms of \emph{efficiency} (i.e., reducing the resource usage maintaining the same functional interface), \emph{efficacy} (i.e., synthesising more stable and faster converging behaviours) and \emph{adaptability} (i.e., the same program works against different environments).
%

As a first contribution, summarised in \Cref{fig:ac-and-rl}, in this work we integrate \ac{rl} within the \ac{ac} control architecture in order to support learning of good collective behaviour sketched by a given aggregate program.
%
Specifically, we focus on improving \ac{ac} building blocks (such as the gradient algorithm covered in \Cref{sec:gradient}) through learning, leading toward a so-called \emph{Reinforcement Learning-based Aggregate Computing}. 
%
...
\subsection{Building blocks Refinement}\label{s:learning-gradient}
...
%
Our idea is that it should not be the designer who codes a particular block to be used, but that it is the learning algorithm that understands, given a global policy to be optimised following a utility function, what actions need to be activated.
...
in a typical \ac{ctde} setting (\Cref{fig:learning-scheme}).
%
...
%
\Cref{lst:general-schema} exemplifies the general program structure used to combine \ac{rl} with \ac{ac} for improving building blocks.
%
...
The gradient block (cf. \Cref{sec:gradient}) could be generalised as follows:
\begin{lstlisting}
def gradientOpt(source, metric, opt) {
  rep(infinity) { g => mux(source) { 0 } { opt(g, metric)) } }
}
\end{lstlisting}
where \lstinline|opt| is a function that determines how to evolve the gradient based on the field of current gradient values \lstinline|g| and current \lstinline|metric| (estimation of distances to neighbours). 
...
In this article, we consider \lstinline|opt| as a \emph{hole} that a \ac{rl} algorithm will fill through raw experience
 with actions aiming at incrementally constructing a gradient field, hopefully mitigating the \emph{rising-value} issue (cf. \Cref{sec:gradient}).
%
To frame our learning problem, we adopt the above-described general schema (\Cref{fig:learning-scheme}). The state and action functions are inspired by the CRF algorithm.
%
...
%
These data have to be discretised; otherwise, the search space would be too big and the solution could suffer from overfitting. 
%
The discretisation is ruled by two variables, $maxBound$ and $buckets$. 
%
...
%
The action space is divided into two action types: \texttt{ConsiderNeighborhood} is the action that will produce the classic gradient evaluation, while 
...
To evaluate our approach, we run a set of simulated experiments and verify that an aggregate system
 can successfully learn an improved way to compute a gradient field (cf. the gradient block described in \Cref{sec:gradient}).
%
...
%
For simulating the slow rising problem, we drop the left source at \SI{35}{\second} ($C_s$), and so the left part of the system starts to rise until eventually stabilising to the correct computational field.
%
...
As learning algorithm, we used Hysteretic Q-Learning (cf. \Cref{s:background:rl}). %, since it tackles environments stochasticity.
%
As behavioural policy, we use an $\epsilon$-greedy with an exponential decay to balance the exploration-exploitation.
%
We make this choice because without using the decay the policy found tends to be not optimised (i.e. the exploration is preferred w.r.t exploitation).
%
At each episode $i$, the $\epsilon$ value is updated as $\epsilon_i = \epsilon_0 \cdot e^{{i} / \theta}$.

Several variables are used, summarised in \Cref{table:parameters}, so we perform a grid search to find the optimal combination.
%
...
\end{equation}
We choose the configuration by observing the box plots (\Cref{subfig:boxplot}) and taking the lowest average error in the last $N_T$ episodes.

\input{papers/coordination2022/simulation-result.tex}
\subsection{Results and Discussion}

\Cref{fig:eval} shows the performance of our \acl{rl}-based gradient algorithm.
%
\Cref{subfig:boxplot} was used to choose which configuration was the best.
%
\Cref{subfig:mean-error-over-episode} shows the error trend as the episodes change. 
%
...
%
In fact, the system eventually learns how to speed up the gradient rising: observing the \Cref{subfig:mean-error-over-episode} the $error_{episode}$ of the new algorithm is lower than the error produced by the standard solution. 
%
...
This intuition is enforced by \Cref{subfig:error-few,subfig:error-many,subfig:output-few,subfig:output-many}.
% 
In particular, in \Cref{subfig:error-few,subfig:output-few} the behaviour is more evident due to the reduced number of agents: when the error is maximised (due to the source that disappears), the error decreased faster than the naive gradient (and, consequently, the output is faster growing).
%
...
%
First of all, the approach could be tuned to learn gradient strategies for smoothness or maximal reactivity in highly variable scenarios, and compared with state-of-the-art algorithms like BIS and ULT~\cite{DBLP:conf/saso/AudritoCDV17}. 
%
