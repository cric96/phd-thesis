...
%
\Cref{s:conc} summarizes results and future research.
...
As a first contribution, summarized in \Cref{fig:ac-and-rl}, in this work we integrate \ac{rl} within the \ac{ac} control architecture in order to support learning of good collective behaviour sketched by a given aggregate program.
%
...
The gradient block (cf. \Cref{sec:gradient}) could be generalized as follows:
\begin{lstlisting}
...
Several variables are used, summarized in \Cref{table:parameters}, so we perform a grid search to find the optimal combination.
%
...
% 
In particular, in \Cref{subfig:error-few,subfig:output-few} the behaviour is more evident due to the reduced number of agents: when the error is maximized (due to the source that disappears), the error decreased faster than the naive gradient (and, consequently, the output is faster growing).
%
